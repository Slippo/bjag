...
void main() 
{
	...
	distance_from_center < 0.9 ? pos.x = pos.x + 0.005*sin(timer + 2*pos.y):pos.x;
	
	// outside circle ? make black. inside? render scene
	distance_from_center > 0.9 ? colour = vec4(0,0,0,1):colour = texture(texture_map, 0.5*pos + 0.5);
	

	// blue tinge only if in the game world
	distance_from_center < 0.9 ? colour = vec4(colour.r - 0.1, colour.g - 0.1, colour.b + 0.2, colour.a):colour = vec4(0,0,0,1);
